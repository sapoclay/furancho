<?php
	...
	function getNumRowsQuery($query) {
		...
		if ($result = $sqlconnection->query($query)) {
			...
			echo "¡Algo salió mal en getNumRowsQuery!";
		}
	}
	...
	function getFetchAssocQuery($query) {
		...
		if ($result = $sqlconnection->query($query)) {
			...
			echo "¡Algo salió mal en getFetchAssocQuery!";
	...
	function getLastID($id, $table) {
		...
		if ($result = $sqlconnection->query($query)) {
			...
			// Si no hay ID en la tabla actualmente
			if ($res[$id] == NULL) {
				return 0;
			}
			return $res[$id];
	...
	function updateTotal($orderID) {
		...
		if ($stmt->execute()) {
			...
			echo "Algo ha salido mal en updateTotal";
			echo $stmt->error;
		}
		$stmt->close();
	}
